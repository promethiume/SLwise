...
def process_data(cell, redic, node_dict):
    ...
    isme_data = pd.read_table(
        ...
        #main_path + '/Data/temp_mutational_exclu_list/{}_me_list.txt'.format(
            cell
        ),
        sep='\t',
    )
    ...
    lowexp_data = pd.read_table(
        ...
        #main_path + '/Data/temp_low_expr_low_geneEffect_list/{}_low_expr_low_geneEffect_list.txt'.format(
            cell
        ),
        sep=',',
    )
    lowexp_data.rename(columns={"rowGene": "gene1", "colGene": "gene2"}, errors="raise")
    par_data = pd.read_table(
        #'/home/intern/SyntheticLethal/SL_project/final_code/Data/{}/p.csv'.format(
        '/home/intern/SyntheticLethal/final_code/Data/{}/p.csv'.format(
            cell
        ),
        sep=',',
    )
    fold_data = pd.read_table(
        ...
        #main_path + '/Data/temp_pair_gene_fold_change_value_list/{}_pair_gene_fold_change_value_list.txt'.format(
            cell
        ),
        sep=',',
    )
